        ...
        <div class="bg-white rounded-xl shadow-sm border border-card-border overflow-hidden">
            <div class="bg-accent-blue/10 p-6 flex flex-col items-center text-center">
                <div class="w-20 h-20 bg-accent-blue text-white rounded-full flex items-center justify-center text-3xl font-black mb-3 shadow-lg">
                    {{ strtoupper(substr($account->candidate_name, 0, 1)) }}
                </div>
                <h2 class="text-xl font-black text-text-main">{{ $account->candidate_name }}</h2>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="pt-4 border-t border-card-border">
                    ...
                    <div class="grid grid-cols-1 gap-y-3">
                        <div>
                            <p class="text-sm font-bold text-text-main">{{ $account->tuition_assigned ?? 'Not Assigned' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-card-border">
                    ...
                    <div class="bg-secondary-bg rounded-lg p-3">
                        <div class="flex justify-between items-center text-xs">
                            ...
                            <span class="font-bold">{{ $account->joining_date ? $account->joining_date->format('M d, Y') : 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center text-xs mt-1">
                            <span class="text-text-dark/60">Monthly Amount:</span>
                            <span class="font-bold text-blue-500 text-sm">₹{{ number_format($account->monthly_amount) }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-card-border overflow-hidden">
            <div class="p-5 border-b border-card-border bg-secondary-bg flex justify-between items-center">
                ...
                <span class="text-xs font-bold bg-accent-blue/10 text-accent-blue px-2 py-1 rounded">{{ $account->payments->count() }} Transactions</span>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-card-border">
                        ...
                        <tr class="hover:bg-secondary-bg/50 transition-colors">
                            ...
                            <td class="px-5 py-3 align-middle">
                                <span class="text-sm font-black {{ $payment->type === 'Collected' ? 'text-green-500' : 'text-blue-500' }}">₹{{ number_format($payment->amount) }}</span>
                            </td>
                            <td class="px-5 py-3 align-middle">
...
<div id="record-payment-modal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <form action="{{ route('admin.candidate-payments.payment.add', $account->id) }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" value="{{ $account->monthly_amount }}" required
                        class="w-full px-4 py-2 bg-white border border-card-border rounded-lg focus:outline-none focus:border-accent-blue focus:ring-1 focus:ring-accent-blue transition-colors">
                </div>
