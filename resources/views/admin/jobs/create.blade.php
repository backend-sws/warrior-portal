        ...
        <form action="{{ route('admin.jobs.store') }}" method="POST" class="p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="subject_id" id="subject_id" data-addable="true" required {{ old('category_id') ? '' : 'disabled' }} class="w-full bg-secondary-bg border border-card-border text-text-main rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-blue/50 focus:border-accent-blue transition-all disabled:opacity-50 cursor-pointer">
...
<script>
    ...
    const jobForm = document.querySelector('form[action*="admin/jobs"]');
    if (jobForm) {
