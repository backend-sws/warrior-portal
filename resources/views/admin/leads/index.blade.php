...
<form id="bulkDeleteForm" action="{{ route('admin.leads.bulk-delete') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete selected contact queries? This cannot be undone.');">
    @csrf
    <div class="bg-card-bg rounded-t-2xl border-x border-t border-card-border p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button type="submit" id="bulkDeleteBtn" style="display: none;" class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-bold transition-all shadow flex items-center gap-1.5 animate-pulse">
                <i class="fas fa-trash-alt text-xs"></i> <span>Delete Selected (<span id="selectedCount">0</span>)</span>
            </button>
        </div>
        <div class="w-full sm:w-auto flex flex-col sm:flex-row items-center justify-end gap-3">
            <div class="relative w-full sm:w-64">
                <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-text-dark/40 text-sm"></i>
                <input type="text" name="search" value="{{ request('search') }}" form="filterForm" placeholder="Search name, email, subject..." 
            ...
            <div class="relative w-full sm:w-auto flex items-center gap-2">
                ...
                <input type="date" name="follow_up_date" value="{{ request('follow_up_date') }}" form="filterForm" title="Filter by Follow-up Date"
            ...
            <button type="submit" form="filterForm" class="w-full sm:w-auto bg-accent-blue text-white rounded-xl px-4 py-2 text-sm font-bold shadow hover:bg-accent-blue-hover transition-colors whitespace-nowrap">Filter</button>
        ...
        <table class="w-full text-left border-collapse admin-table">
            <thead>
                <tr>
                    <th class="w-10 text-center">
                        <input type="checkbox" id="selectAllCheckbox" onchange="toggleSelectAll(this)" class="w-4 h-4 rounded border-card-border text-accent-blue focus:ring-accent-blue/40 cursor-pointer">
                    </th>
                    @php
            ...
            <tbody class="divide-y divide-card-border">
                ...
                <tr class="group hover:bg-secondary-bg/30 transition-colors">
                    <td class="align-top text-center pt-4">
                        <input type="checkbox" name="ids[]" value="{{ $lead->id }}" onchange="updateSelectedCount()" class="lead-checkbox w-4 h-4 rounded border-card-border text-accent-blue focus:ring-accent-blue/40 cursor-pointer">
                    </td>
                    <td class="align-top">
                    ...
                    <td class="align-top">
                        <div class="flex items-center justify-end gap-1.5">
                            ...
                            <button type="button" onclick="deleteSingleLead('{{ route('admin.leads.destroy', $lead->id) }}')" class="w-8 h-8 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors shadow-sm" title="Delete Inquiry">
...
<form id="filterForm" action="{{ route('admin.leads.index') }}" method="GET" style="display: none;"></form>
