...
<div class="bg-primary-bg min-h-screen pb-12">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-text-main">My Tuitions</h1>
                <p class="text-text-dark/70 text-sm mt-1">Manage all your home tuition requirements.</p>
            </div>
            <a href="{{ route('employer.tuitions.create') }}" class="bg-accent-yellow hover:bg-accent-yellow/90 text-primary-bg px-5 py-2.5 rounded-lg text-sm font-semibold transition-all">
        ...
        <div class="bg-card-bg border border-card-border rounded-xl shadow-sm overflow-hidden">
            @if($tuitions->count() > 0)
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-primary-bg/50 border-b border-card-border text-xs uppercase tracking-wider text-text-dark/70">
                        ...
                        <tbody class="divide-y divide-card-border">
                            @foreach($tuitions as $tuition)
                                <tr class="hover:bg-primary-bg/20 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="font-medium text-text-main">{{ $tuition->subjects }}</div>
                                        <div class="text-xs text-text-dark/70 mt-0.5">{{ $tuition->student_class }} • {{ $tuition->board }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-text-main">{{ $tuition->location }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-text-main">{{ $tuition->budget }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                ...
                <div class="px-6 py-4 border-t border-card-border bg-primary-bg/20">
                    {{ $tuitions->links() }}
                </div>
            @else
                <div class="p-12 text-center">
                    <div class="w-16 h-16 bg-primary-bg rounded-full flex items-center justify-center mx-auto mb-4 border border-card-border">
                        <i class="fas fa-chalkboard-teacher text-2xl text-text-dark/50"></i>
                    </div>
                    <h3 class="text-lg font-semibold text-text-main mb-1">No Tuitions Posted Yet</h3>
                    <p class="text-text-dark/70 text-sm mb-6">You haven't posted any home tuition requirements yet.</p>
