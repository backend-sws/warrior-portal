...
@section('title', 'Apply as Home Tutor | Warriors Educare')
@section('meta_description', 'Register as a verified home tutor with Warriors Educare. Teach students in your preferred localities in Patna and earn top tuition fees.')

@section('content')
<div class="bg-[#f8fafc] text-slate-900 py-8 sm:py-12" x-data="homeTutorForm()">
    <div class="max-w-4xl mx-auto px-4 sm:px-6">
        
        {{-- Hero Header --}}
        <div class="text-center mb-8">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-amber-800 text-xs font-extrabold uppercase tracking-wider mb-3 shadow-2xs">
                <i class="fas fa-home text-sm text-amber-600"></i> Home Tutor Registration
            </div>
            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black text-[#031b4e] tracking-tight">
                Apply as a Private Home Tutor
            </h1>
            <p class="text-sm sm:text-base text-slate-600 max-w-2xl mx-auto mt-2">
                Join our network of verified home educators. Connect directly with parents, choose your local areas, and start teaching offline or online.
            </p>

            {{-- Quick Switcher Cards --}}
            <div class="mt-5 flex flex-wrap items-center justify-center gap-2">
                <span class="text-xs font-bold text-slate-400">Looking for another form?</span>
                <a href="{{ route('apply.school-teacher') }}" class="text-xs font-bold text-[#031b4e] hover:text-blue-600 bg-white border border-slate-200 hover:border-blue-300 px-3 py-1 rounded-full transition-all shadow-2xs">
                    🏫 School Teaching Job
                </a>
            ...
            <form @submit.prevent="submitForm($event)" enctype="multipart/form-data" class="p-6 sm:p-8 lg:p-10 space-y-6">
                ...
                <div class="pt-4 border-t border-slate-100 space-y-4">
                    ...
                    <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                        ...
                        @php $tuitionSubsList = ['Pre-Primary', 'All Subjects', 'Mathematics', 'Science', 'Physics', 'Chemistry', 'Biology', 'English', 'Hindi', 'SST', 'Computer', 'Spoken English', 'Accounts', 'Economics', 'Business Studies']; @endphp
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4">
                        ...
                        @php $classList = ['Pre-Primary', 'Class 1-5', 'Class 6-8', 'Class 9-10', 'Class 11-12', 'IIT-JEE', 'NEET', 'Olympiad', 'Competitive / Olympiad', 'Languages / Hobby']; @endphp
                    ...
                    {{-- Preferred Areas Card (MANUAL TEXTAREA WITH QUICK BUTTONS & GPS) --}}
                    <div class="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <button type="button" @click="detectLocation(false)" class="text-[11px] font-bold text-emerald-800 hover:text-emerald-950 bg-white hover:bg-emerald-100 px-2.5 py-1 rounded-lg border border-emerald-300 transition-colors flex items-center gap-1 cursor-pointer">
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-2">
                            @foreach(['Patna', 'Kankarbagh', 'Boring Road', 'Bailey Road', 'Danapur', 'Rajendra Nagar', 'Anisabad'] as $quickArea)
                                <button type="button" @click="appendArea('{{ $quickArea }}', 'ht_areas')" class="text-[11px] font-bold bg-white hover:bg-emerald-100 text-emerald-700 border border-emerald-300 px-2.5 py-1 rounded-lg cursor-pointer transition-colors">
                                    + {{ $quickArea }}
                                </button>
                            @endforeach
                        </div>
                        <span x-show="fieldErrors['preferred_areas']" x-text="fieldErrors['preferred_areas'] ? fieldErrors['preferred_areas'][0] : ''" class="text-rose-500 text-[11px] font-bold mt-1 block"></span>
                    </div>
                ...
                <div class="pt-4 border-t border-slate-100">
                    <button type="submit" :disabled="submitting"
                        ...
                        <span x-show="!submitting" class="flex items-center justify-center gap-2 text-white font-bold">
                            <span>Complete Tutor Registration & Send OTP</span>
