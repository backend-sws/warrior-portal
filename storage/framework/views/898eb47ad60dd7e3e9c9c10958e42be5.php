...
<div id="jobRegPopup" class="fixed inset-0 hidden items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm opacity-0 transition-opacity duration-500" style="z-index: 99999;">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative transform transition-transform duration-500 popup-content" style="transform: scale(0.95);">
        ...
        <div class="p-6 bg-slate-50">
            ...
            <?php endif; ?>
            <form action="<?php echo e(route('candidate.register.post')); ?>" method="POST" class="space-y-3">
                <?php echo csrf_field(); ?>
                <div>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><i class="fas fa-user text-sm"></i></span>
                        <input name="name" type="text" required class="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-accent-blue" placeholder="Full Name" value="<?php echo e(old('name')); ?>">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><i class="fas fa-envelope text-sm"></i></span>
                        <input name="email" type="email" required class="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-accent-blue" placeholder="Email" value="<?php echo e(old('email')); ?>">
                    </div>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><i class="fas fa-phone-alt text-sm"></i></span>
                        <input name="phone" type="text" required class="w-full bg-white border border-slate-200 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-accent-blue" placeholder="Phone Number" value="<?php echo e(old('phone')); ?>">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="relative">
            ...
            <div class="mt-3">
                <a href="<?php echo e(route('resume.builder')); ?>" class="w-full bg-white border-2 border-accent-blue text-accent-blue font-bold py-2 rounded-lg hover:bg-blue-50 transition-colors flex items-center justify-center gap-2">
                    <i class="fas fa-file-pdf"></i> Try Free Resume Builder
                </a>
            </div>

            <div class="mt-4 text-center border-t border-slate-200 pt-4">
                <p class="text-xs text-slate-500">Already have an account? <a href="<?php echo e(route('login')); ?>" class="text-accent-blue font-bold hover:underline">Login here</a></p>
...
<?php endif; ?>
<?php /**PATH D:\warrior-portal\resources\views/partials/job-registration-popup.blade.php ENDPATH**/ ?>